...
import org.jug.nicaragua.picoweb.view.event.PICOWEBEvent.NotificationsCountUpdatedEvent;
import org.jug.nicaragua.picoweb.view.event.PICOWEBEvent.PostViewChangeEvent;
import org.jug.nicaragua.picoweb.view.event.PICOWEBEvent.ProfileUpdatedEvent;
import org.jug.nicaragua.picoweb.view.event.PICOWEBEvent.ReportsCountUpdatedEvent;
import org.jug.nicaragua.picoweb.view.event.PICOWEBEvent.TransactionReportEvent;
...
import org.jug.nicaragua.picoweb.view.forms.ProfilePreferencesWindow;
...
public final class DashboardMenu extends CustomComponent {
  ...
  private Component buildTitle() {
    Label logo = new Label("<strong><B>RRHH 2019.</B></strong>", ContentMode.HTML);
    // logo.setSizeUndefined(); // TODO altura
  ...
  private Component buildUserMenu() {
    ...
    settingsItem.addItem("Salir", new Command() {
      @Override
      public void menuSelected(final MenuItem selectedItem) {
        PICOWEBEventBus.post(new UserLoggedOutEvent());
      }
    });
    return settings;
  }
  ...
  /**
   * Boton del usuario en linea actualmente.
   * 
   * @return
   */
  ...
  private Component buildMenuItems() {
    ...
    for (final PantallasViewType view : PantallasViewType.values()) {
      Component menuItemComponent = new ValoMenuItemButton(view);

      if (view == PantallasViewType.REPORTS) {
        // Add drop target to reports button
        DragAndDropWrapper reports = new DragAndDropWrapper(menuItemComponent);
        reports.setSizeUndefined(); // TODO altura
        reports.setDragStartMode(DragStartMode.NONE);
        reports.setDropHandler(new DropHandler() {

          @Override
          public void drop(final DragAndDropEvent event) {
            UI.getCurrent().getNavigator().navigateTo(PantallasViewType.REPORTS.getViewName());
            Table table = (Table) event.getTransferable().getSourceComponent();
            PICOWEBEventBus.post(new TransactionReportEvent((Collection<Transaction>) table.getValue()));
          }
      ...
      if (view == PantallasViewType.DASHBOARD) {
  ...
  private Component buildBadgeWrapper(final Component menuItemButton, final Component badgeLabel) {
    ...
    badgeLabel.setWidthUndefined(); // TODO altura
  ...
  public void updateNotificationsCount(final NotificationsCountUpdatedEvent event) {
    int unreadNotificationsCount = PICOWEBUI.getDataProvider().getUnreadNotificationsCount();
  ...
  public final class ValoMenuItemButton extends Button {
    ...
    private final PantallasViewType view;

    public ValoMenuItemButton(final PantallasViewType view) {
      this.view = view;
