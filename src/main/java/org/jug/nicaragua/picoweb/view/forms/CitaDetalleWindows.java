...
public final class CitaDetalleWindows extends Window {

  private final Label synopsis = new Label();

  private CitaDetalleWindows(final Movie movie, final Date startTime, final Date endTime) {
    addStyleName("moviedetailswindow");
    Responsive.makeResponsive(this);

    setCaption("Cita del señor Juan Perez");
    center();
  ...
  private Component buildFooter() {
    ...
    reprogramar.addStyleName(ValoTheme.BUTTON_PRIMARY);
    Button cancelar = new Button("Cancelar");
    cancelar.addStyleName(ValoTheme.BUTTON_PRIMARY);
    Button facturar = new Button("Facturar");
    facturar.addStyleName(ValoTheme.BUTTON_PRIMARY);
    Button examenes = new Button("Examenes");



    ok.focus();
    footer.addComponents(facturar, cancelar, reprogramar, ok);
  ...
  private Component buildDetailsForm(final Movie movie, final Date startTime, final Date endTime) {
    ...
    if (startTime != null) {
      ...
      label.setCaption("Fecha");
      fields.addComponent(label);

      df.applyPattern("hh:mm a");
      label = new Label(df.format(startTime));
      label.setSizeUndefined();
      label.setCaption("Hora");
      fields.addComponent(label);
    }
  ...
  public static void open(final Movie movie, final Date startTime, final Date endTime) {
    ...
    Window w = new CitaDetalleWindows(movie, startTime, endTime);
